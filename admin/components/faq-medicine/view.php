...
<?php } ?>							
	...
	<div class="row flex-lg-nowrap">
							...
							<div class="row">
											...
											<div class="col-md-12 col-lg-12 col-xl-3">
												<div class="form-group">
                                                    ...
                                                    <?php
														$sqlCategory="select * from tbl_medication where med_status=1 order by med_title";
														$resCategory=$database->get_results($sqlCategory);
														
													
													?>
                                                    
													<select name="cmbCategory"  class="form-control custom-select select2" data-placeholder="All Medication">
														<option label="All Medication"></option>
                                                       
                                                        <?php for ($c=0;$c<count($resCategory);$c++)
														{
															$rowCategory=$resCategory[$c];
															?>
														<option value="<?php echo $rowCategory['med_id']?>" <?php if ($rowCategory['med_id']==$_GET['cmbCategory']) echo "selected"; ?>><?php echo $rowCategory['med_title']?></option>
														<?php } ?>
													</select>
												</div>
											</div>
		...
		<input type="checkbox" class="custom-control-input" name="chkControl" value="yes" onClick="checkAll(this.form,this.checked);" />
                                                ...
                                                <th class="border-bottom-0 w-60">Medicine</th>
				...
				<input name="deletes[]" id="chkDelete" onClick="isChecked(this.checked);" class="custom-control-input" value="<?php echo $row['faq_id']; ?>" type="checkbox"  />			
                                    ...
                                    <td class="align-middle">
										<?php echo getMedicineName($row['faq_category']); ?>
...
<div class="page-header d-lg-flex d-block">
	<div class="page-leftheader">
	<h4 class="page-title">FAQs for medicine : <?php if (@count($row)>0) echo 'Edit'; else echo 'Add'; ?></h4>
	</div>
	<div class="page-rightheader ml-md-auto">
		<div class=" btn-list">
   ...
   <div class="card-body pb-2">
							...
							<div class="form-group">
								<label class="form-label">Medicine</label>
									...
									<select class="form-control" name="faq_conditions" id="faq_conditions"  >
										<option label="Select Medicine"></option>
										<?php
				...
				$results = $database->get_results( $query );
						...
						foreach ($results as $value) {
								...
								<option value="<?php echo $value['med_id']; ?>"  <?php if($row['faq_category'] == $value['med_id']) {	echo 'selected="selected"';}?>  ><?php echo $value['med_title']; ?></option>
