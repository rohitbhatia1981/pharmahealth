...
if (($_POST['txtOldPassword']!="") && ($_POST['txtPassword']!="") && ($_POST['txtCPassword'] == $_POST['txtPassword']) ) {
				 ...
				 $userId=$_SESSION['sess_prescriber_id'];


				 $sql = "SELECT * FROM tbl_prescribers where pres_id='".$database->filter($userId)."' and pres_password='".$database->filter($oldpassword)."'";
		...
		'pres_id' => $userId
			...
			$updated = $database->update( 'tbl_prescribers', $update, $where_clause );
			...
			if( $updated )
			{
				...
				$receiverName=$rowMemberid['pres_forename']." ".$rowMemberid['pres_surname'];
				$email=$rowMemberid['pres_email'];
