<?php
	...
	global $database, $page, $pagingObject;
		  ...
		  $sql = "SELECT * FROM tbl_tickets WHERE message_sender_type='Clinician' and message_parent=0 and message_sender_id='".$database->filter($_SESSION['sess_prescriber_id'])."'";
		
		if($_GET['txtSearch'] != "")
		{
			$sql .= " and (message_subject like '%".$database->filter($_GET['txtSearch'])."%' || message_id like '%".$database->filter($_GET['txtSearch'])."%' || message_text like '%".$database->filter($_GET['txtSearch'])."%' || message_id like '%".$database->filter($_GET['txtSearch'])."%') ";
	...
	$fileCount = count($_FILES['flDoc']['name']);
		...
		if ($hid>0)
		{
		$updateStatus="update tbl_tickets set message_replier_status=0,message_read_status_admin=0 where message_id='".$database->filter($hid)."'";
		$database->query($updateStatus);
				...
				//----------Creating log--------
					...
					$utype="clinician";
					$action=$name." has sent message to admin";
