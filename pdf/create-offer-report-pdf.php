...
<p>&nbsp;</p>
<h2 style="font-size:30px" align="center">Expression of Interest</h2>
<p>&nbsp;</p>
...
<table width="100%" border="1" cellspacing="5" cellpadding="5" style="border-collapse:collapse">
  <tr style="background-color:#F1F3FB">
	...
	$faxBody.='<td style="font-size:17px"><strong>Offered Price</strong></td>
...
$rowOffer=$resultOffer[$p];
 ...
 $faxBody.=' <tr style="font-size:16px"> 
 	<td>'.$sno.'</td> 	
	<td>'.$rowOffer['offer_name'].'</td>';
		
	if ($phoneRep==1)
	$faxBody.='<td>'.$rowOffer['offer_phone'].'</td>';
	
	if ($emailRep==1)
	$faxBody.='<td>'.$rowOffer['offer_email'].'</td>';
	
	$faxBody.='<td style="color:#0000FF; font-size:17px">$'.number_format($rowOffer['offer_price']).'</td>  
	<td>'.$rowOffer['offerDate'].'</td>
  </tr>';
  $sno=$sno+1;
}
...
file_put_contents(PATH.'agents/reports/'.$fileId.'.pdf', $dompdf->output());
