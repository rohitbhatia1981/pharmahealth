...
function createThumbnail($imageDirectory, $imageName, $thumbDirectory, $thumbWidth, $quality = 100)
{
    ...
    $thumbPath = "$thumbDirectory/$imageName";

    // Create an image resource from the original image
    $srcImg = null;
    $image_extension = strtolower(pathinfo($imagePath, PATHINFO_EXTENSION));
    ...
    // Create a truecolor image for the thumbnail
    $thumbImg = imagecreatetruecolor($thumbWidth, $thumbHeight);

    // Copy and resample the original image to create the thumbnail
...
function rotateImage($image_name, $path, $degree_lvl)
{
    ...
    {
        case "jpg": 
            $rotated_image_name = "$path/$image_name";
            header('Content-type: image/jpeg');
            imagejpeg($rotated_image, $rotated_image_name, 100);
            break;
        case "jpeg":
            $rotated_image_name = "$path/$image_name";
            header('Content-type: image/jpeg');
        ...
        case "png":
            ...
            imagepng($rotated_image, $rotated_image_name);
            break;
    }
    
    // Append timestamp to image URL
    $timestamp = time();
    return "$rotated_image_name?$timestamp";
}
