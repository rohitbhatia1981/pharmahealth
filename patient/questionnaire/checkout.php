               ...
               <div class="row">
                   <div class="col-sm-7 left">
					   ...
					   for ($k=0;$k<count($_SESSION['sessCart']);$k++)
					   {
						   ...
						   ?>
                           <li><?php echo getMedicineName($_SESSION['sessCart'][$k]['med_id']) ?> <span><?php echo CURRENCY ?><?php echo formatToTens($price); ?></span>
                           ...
                           <li><a href="javascript:;"  class="openModalBtn" data-modal-id="myModal2" data-medicine-id="<?php echo $_SESSION['sessCart'][$k]['med_id']?>" data-index-id="<?php echo $k?>" style="text-decoration:underline">Amend Quantity / Strength</a>
                           <!--<li>Pack Quantity: 
					...
					if (count($resCommon)>0)
					{
						...
						if (isset($_SESSION['sessCart_common'])) {
							foreach ($_SESSION['sessCart_common'] as $item) {
								if (isset($item['med_id']) && $item['med_id'] == 5) {
									$disable=17;
									break; // Exit the loop once the value is found
								}
								if (isset($item['med_id']) && $item['med_id'] == 17) {
									$disable=5;
									break; // Exit the loop once the value is found
								}
							}
				 ...
				 ?>
                     <div class="commonly_section" style="margin-top:10px; padding:0px !important">
                       <h5 style="font-size:16px;font-weight:bold;color:#333">Commonly bought with...</h5>
						...
						if ($strAnd!="")
						{
						for ($j=0;$j<count($resCommon);$j++)					
						{
							$rowCom=$resCommon[$j];
							
							if ($disable==$rowCom['med_c_id'])
                            ...
                            <td width="25%"><img src="<?php echo URL?>classes/timthumb.php?src=<?php echo URL?>images/medication/common/<?php echo $rowCom['med_c_image']; ?>&w=50&h=50&zc=2"></td>
                            	<td width="67%"  style="font-size:15px">
                                <?php echo $rowCom['med_c_title']?></td>
                               	<td width="2%"><?php echo CURRENCY.formatToTens($rowCom['med_c_price'])?></td>
                                <td width="35%" >
...
If your prescription request is rejected by our clinical team, the authorization will be canceled and no charges will be made. If an alternative treatment is available, our clinical team will contact you.</p>
				...
				$curDate=date("Y-m-d");
				$sqlHoliday="select * from tbl_bank_holidays where holiday_date='".$curDate."'";
				$resHoliday=$database->get_results($sqlHoliday);
				$ctrRec=count($resHoliday);
				if ($ctrRec>0)
				$onTime=0;
