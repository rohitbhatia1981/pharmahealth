    ...
    <?php include "include/step-navigation.php"; ?>
         ...
         <div class="setup_white_box mt-4">
             ...
             <?php
			 	...
			 	$sqlQ = "SELECT * FROM tbl_medical_questions where mq_status=1 and mq_category=4 and mq_conditions='".$database->filter($_SESSION['sessCondition'])."' order by mq_id asc";
				 ...
				 ?>
                     ...
                     <div class="form-group">
							...
							for ($j=0;$j<count($arrOptions);$j++)
							{
                                ...
                                <img src="<?php print URL;?>classes/timthumb.php?src=<?php echo URL?>uploads/questionnaire/<?php echo $imageOpt ?>&w=280&h=220&zc=2" style="vertical-align: middle;padding:30px">
                                 ...
                                 <?php if ($rowQ['mq_ask_for_information']==$optIn){ ?>
                                    	<div><textarea placeholder="Please provide more information" rows="3" cols="30" class="<?php echo $textAreClass;?>" style="display:none; padding:10px; margin-left:10pxbackground-color: #fbfcfc; min-height: 180px; width:100%; border-radius: 2px; resize: none;" name="txtMore_<?php echo $rowQ['mq_id']?>" id="txtMore_<?php echo $rowQ['mq_id']?>_<?php echo $optIn?>"></textarea></div>
                                    <?php } ?>
                            <?php } ?>
						 ...
						 ?>
                                ...
                                <div>
                                    <textarea class="form-control" rows="4" style="max-height:50px !important" name="txt_<?php echo $rowQ['mq_id']?>" placeholder="Plese provide information"></textarea>
                                </div>
                             </li>
							...
							return false;
							 ...
							 $("#submitBtn").html("Please wait..</div>");
								...
								var fd = new FormData(myform);	
								   ...
								   $.ajax({		
								   type: "POST",			
								   url: "<?php echo URL?>patient/questionnaire/ajax/insert-step5.php",			
								   data: fd,			
								   cache: false,			
								   processData: false,			
								   contentType: false,						   
								  ...
								  //$("#error-container").html(msg);		
									if (msg==1)			
									  window.location='<?php echo URL?>patient/questionnaire/step6';	
									   else if (msg==2)			
									   {
