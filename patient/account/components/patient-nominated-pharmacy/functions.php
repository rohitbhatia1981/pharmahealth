...
$todaysDate=$curDate=date("Y-m-d H:i:s");		


$names = array(
'pr_patient_id' => $_SESSION['sess_patient_id'],
'pr_old_pharmacy' => $_SESSION['sessOldPharmacy'],
'pr_new_pharmacy' => $_POST['rdPharmacy'],
'pr_reason' => $_POST['txtReason'],
'pr_date' => $todaysDate,
'pr_status' => 0,

);

$add_query = $database->insert( 'tbl_pharmacy_request', $names );
	...
	//------------sending email to admin----
		...
		$sqlEmail="select * from tbl_emails where email_id=60 and email_status=1";
		$resEmail=$database->get_results($sqlEmail);
				...
				$SubjectSend="Change pharmacy request by patient";
				$BodySend=$mailBody;	
